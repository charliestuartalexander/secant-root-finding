...
public class Project2 {
    ...
    public int index(Complex root) {
        int k = -1;
        for(int i=0;i<roots.size();i++){
            // Check if polynomials are equal (within TOL)
            if(root.add(roots.get(i).negate()).abs() < Secant.TOL){
                k=i;
                break;
            }
        }
    ...
    public Complex pixelToComplex(int i, int j) {
        ...
        return origin.add((new Complex(i,-j)).multiply(dz));
    }
    ...
    public void createFractal(boolean colourIterations) {
        ...
        // Copy colour Iterations to instance variable
        Complex root = new Complex();
        for (int i = 0; i < NUMPIXELS; i++){
            for (int j = 0; j < NUMPIXELS; j++){
                // iterate over each pixel
                iterator.iterate(new Complex(),pixelToComplex(i,j)) ;
                if ((index(iterator.getRoot()) == -1 || roots.size()==0) && iterator.getError() == Secant.Error.OK){
    ...
    public static void main(String[] args) {
        ...
        Complex[] coeff2 = new Complex[] { new Complex(0,1),  new Complex(), new Complex(1.3,0.2), new Complex(0.2,2), new Complex(-1,2), new Complex(1,1) };
        Polynomial p2    = new Polynomial(coeff2);
        Project2 project2 = new Project2(p2, new Complex(-1.0,1.0), 2);
        
        project2.createFractal(false);
        project2.saveFractal("fractal-light2.png");
        project2.createFractal(true);
        project2.saveFractal("fractal-dark2.png");
    }
